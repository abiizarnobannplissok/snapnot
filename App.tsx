...
export default function App() {
  ...
  return (
    <div ref={mainContainerRef} className="min-h-screen pb-20">
      {/* Header */}
      <header className="sticky top-0 z-30 bg-[#F5F5F5]/90 backdrop-blur-md border-b border-border py-5 px-4 md:px-8">
        <div className="max-w-[1400px] mx-auto w-full">
          <div className="flex flex-col gap-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between md:gap-6 gap-4">
              ...
              <div className="flex items-center justify-between gap-4 flex-shrink-0">
                <div className="flex items-center gap-3">
                  ...
                  <div className="flex items-center gap-3">
                    <div>
                      <h1 className="text-2xl md:text-3xl font-bold tracking-tight">SnapNotes</h1>
                      <p className="text-sm text-text-secondary mt-0.5 whitespace-nowrap">Notes File Sharing and Translate</p>
                    </div>
                    {activeTab === 'notes' && (
                      <div className="bg-white px-4 py-2 rounded-2xl shadow-sm border border-gray-100 md:hidden flex flex-col items-end">
                ...
                {activeTab === 'notes' && (
                  <button
                    ...
                    className={`md:hidden h-9 px-4 text-sm font-bold rounded-xl transition-all active:scale-95 shadow-sm whitespace-nowrap flex-shrink-0 ${
                      noteSelectionMode 
                ...
                <div className="hidden md:flex gap-2 bg-white p-1.5 rounded-2xl shadow-sm border border-gray-100">
                  <button
                    ...
                    className={`
                      h-9 flex items-center gap-2 px-4 rounded-xl text-sm font-medium transition-all whitespace-nowrap
                      ${activeTab === 'notes'
                ...
                {activeTab === 'notes' ? (
                  <>
                    <div className="relative group w-full md:w-[240px]">
                      <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-text-secondary w-4 h-4 group-focus-within:text-black transition-colors" />
                      <input 
                        type="text" 
                        placeholder="Cari catatan..." 
                        value={searchQuery}
                        onChange={(e) => setSearchQuery(e.target.value)}
                        className="w-full h-11 pl-10 pr-4 text-sm rounded-2xl bg-white border border-transparent focus:border-black focus:outline-none shadow-soft transition-all placeholder:text-gray-400"
                      />
                    </div>
                    ...
                    <button 
                      ...
                      className="hidden md:inline-flex h-11 items-center justify-center gap-2 bg-primary px-5 rounded-2xl text-sm font-bold text-black hover:scale-105 active:scale-95 transition-transform shadow-[0_4px_12px_rgba(212,255,0,0.4)] whitespace-nowrap flex-shrink-0"
                    >
                  ...
                  <>
                    <div className="relative group w-full md:max-w-[260px]">
                      <Search className="absolute left-3.5 top-1/2 -translate-y-1/2 text-text-secondary w-4 h-4 group-focus-within:text-black transition-colors" />
                      <input 
                        type="text" 
            ...
            <div className="flex md:hidden gap-1.5 bg-white p-1.5 rounded-2xl shadow-sm border border-gray-100">
              <button
                onClick={() => setActiveTab('notes')}
                className={`
        ...
        <div className="hidden md:flex items-center justify-between mb-8">
          <div className="flex items-center gap-2 text-text-secondary">
            ...
            <span className="font-medium text-sm">
              ...
              {filteredNotes.length !== notes.length && (
                <span className="ml-1.5 text-black font-semibold">(Menampilkan {filteredNotes.length} hasil)</span>
              )}
          ...
          <div className="flex items-center gap-3">
            {noteSelectionMode && selectedNotes.size > 0 && (
              <button
                ...
                className="h-9 px-5 text-sm bg-red-500 text-white font-bold rounded-xl hover:bg-red-600 active:scale-95 transition-all flex items-center gap-2 shadow-sm"
              >
            ...
            <button
              onClick={toggleNoteSelectionMode}
              className={`h-9 px-5 text-sm font-bold rounded-xl transition-all active:scale-95 shadow-sm ${
                noteSelectionMode 
        ...
        {noteSelectionMode && (
          <div className="md:hidden flex items-center justify-between mb-4 px-1">
            ...
            {selectedNotes.size > 0 && (
              <button
                onClick={handleDeleteSelectedNotes}
                className="h-8 px-4 text-sm bg-red-500 text-white font-bold rounded-xl active:scale-95 transition-all flex items-center gap-2 shadow-sm"
              >
                <Trash2 className="w-4 h-4" />
                Hapus ({selectedNotes.size})
              </button>
          ...
          <div className="flex flex-col items-center justify-center py-20 text-center animate-fade-in">
            ...
            {notes.length === 0 && (
              <button 
                ...
                className="bg-black text-white px-8 py-3 text-sm rounded-2xl font-medium hover:bg-gray-800 transition-colors"
              >
